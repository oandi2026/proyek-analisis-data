...
# Dynamic mapping configuration for files and colors
commodity_mapping = {
    "Milk": {
    ...
    "Honey": {
        ...
        "value_col": "Value",  # Matches your precise column name!
...
try:
    ...
    # Automatic Column Fallback Check
    val_col = current_config["value_col"]
    if val_col not in df.columns and "Value" in df.columns:
        val_col = "Value"
    elif val_col not in df.columns:
        val_col = df.columns[-1]

    # Standardize string numeric entries into clean floats
    ...
    top_state_ansi = df_sorted_desc.iloc["State_ANSI"]
    top_state_val = df_sorted_desc.iloc[val_col]
    ...
    with col2:
        ...
        # Sort values ascending for horizontal layout bars
        df_sorted_asc = df.sort_values(by=val_col, ascending=True)

        fig, ax = plt.subplots(figsize=(10, 5))
        
        # Axis abbreviator helper function
        ...
        # Render custom colored bars based on current commodity selection
    ...
    st.error(
        f"Error rendering dashboard. Please verify that your CSV datasets are correctly named "
        f"and saved inside the 'data/' folder. Error Details: {e}"
    )
